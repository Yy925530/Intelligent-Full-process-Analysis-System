...
if date_col and date_col in df_clean.columns:
    ...
    if not df_date_valid.empty:
        ...
        if not df_date_valid.empty:
            ...
            axes[0].set_title("Monthly Sales Trend", fontsize=10)
            axes[0].set_xlabel("Month", fontsize=8)
            axes[0].set_ylabel("Sales", fontsize=8)
            axes[0].grid(True, alpha=0.3)
            
            if monthly_quantity is not None:
                monthly_quantity.plot(ax=axes[1], marker='s', color='#ff7f0e', linewidth=2)
                axes[1].set_title("Monthly Quantity Trend", fontsize=10)
                axes[1].set_xlabel("Month", fontsize=8)
                axes[1].set_ylabel("Quantity", fontsize=8)
...
with col2:
    if "销量" in df_clean.columns and not df_clean["销量"].dropna().empty:
        ...
        ax.set_title("Quantity Distribution", fontsize=10)
...
if region_col:
    ...
    with col1:
        if not region_sales.empty:
            ...
            ax.set_title("Sales by Region", fontsize=10)
...
if product_col:
    ...
    if not product_sales.empty:
        ...
        ax.set_title("Top 10 Products", fontsize=10)
    ...
    with col1:
        if category_col:
            ...
            if not category_sales.empty:
                ...
                ax.set_title("Category Sales", fontsize=10)
                ax.set_ylabel("")
                plt.tight_layout(pad=0.3)
                st.pyplot(fig, use_container_width=False)
    ...
    with col2:
        if gender_col:
            ...
            if not gender_sales.empty:
                ...
                ax.set_title("Gender Sales", fontsize=10)
                ax.set_ylabel("")
                plt.tight_layout(pad=0.3)
                st.pyplot(fig, use_container_width=False)
...
if product_col:
    ...
    if is_moon or is_choco:
        ...
        if is_moon:
            ...
            df_clean["口味"] = df_clean[product_col].apply(extract_choco_flavor)   # 修正拼写
        ...
        df_clean["口味_英文"] = df_clean["口味"].map(flavor_en_map).fillna('Other')

        # 使用英文口味分组绘图
        flavor_sales = df_clean.groupby("口味_英文")["销售额"].sum().sort_values(ascending=False)

        if not flavor_sales.empty:
            fig, ax = plt.subplots(figsize=(5, 3), dpi=120)
            flavor_sales.head(10).plot(kind="bar", color="coral", ax=ax)
            ax.set_title("Flavor Sales", fontsize=10)
            ax.set_xlabel("Flavor", fontsize=8)
            ax.set_ylabel("Sales", fontsize=8)
            plt.xticks(rotation=75, ha="right", fontsize=8)
            plt.tight_layout(pad=0.3)
            st.pyplot(fig, use_container_width=False)
            top_flavor = flavor_sales.index[0]
            st.success(f"📌 **Most Popular Flavor: {top_flavor}**, Sales Share {flavor_sales.iloc[0]/flavor_sales.sum()*100:.1f}%")
            st.write("**Insight:** Top flavor dominates the market; niche flavors have limited demand.")
            st.info("**Action:** Increase production for top flavors; small batches for niche flavors to reduce inventory.")
...
if age_col:
    ...
    if not df_age_valid.empty:
        ...
        ax.set_title("Age Group Sales", fontsize=10)
...
if salesperson_col:
    ...
    if not salesperson_sales.empty:
        ...
        ax.set_title("Top 10 Salesperson", fontsize=10)
        ax.set_xlabel("Sales", fontsize=8)
        ax.set_ylabel("Sales", fontsize=8)
        plt.xticks(rotation=45, ha="right", fontsize=8)
        plt.tight_layout(pad=0.3)
        st.pyplot(fig, use_container_width=False)
...
if channel_col:
    ...
    if not channel_sales.empty:
        ...
        ax.set_title("Channel Sales", fontsize=10)
    ...
    with col1:
        if pay_col:
            ...
            if not pay_sales.empty:
                ...
                ax.set_title("Payment Method", fontsize=10)
    ...
    with col2:
        if customer_type_col:
            ...
            if not customer_sales.empty:
                ...
                ax.set_title("Customer Type", fontsize=10)
                ax.set_ylabel("",fontsize=8)
                plt.tight_layout(pad=0.3)
                st.pyplot(fig, use_container_width=False)
...
# ==================== 路径2：相关性与影响因素 ====================
st.divider()
st.subheader("Path 2: Correlation & Influencing Factors Analysis")

numeric_cols = df_clean.select_dtypes(include=[np.number]).columns.tolist()
numeric_cols_for_corr = [c for c in numeric_cols if c not in ["地区_编码", "产品类别_编码"]]

if len(numeric_cols_for_corr) > 1:
    st.markdown("### Numerical Variable Correlation Heatmap")
    
    # --- Map Chinese column names to English for heatmap display ---
    colname_map = {
        '价格': 'Price',
        '销量': 'Quantity',
        '销售额': 'Sales',
    ...
    # Only rename columns that exist in numeric_cols_for_corr
    rename_dict = {k: v for k, v in colname_map.items() if k in numeric_cols_for_corr}
    if rename_dict:
        df_heat = df_clean.rename(columns=rename_dict)
        renamed_cols = [rename_dict.get(c, c) for c in numeric_cols_for_corr]
    else:
        df_heat = df_clean
        renamed_cols = numeric_cols_for_corr
    
    corr_matrix = df_heat[renamed_cols].corr()
    fig, ax = plt.subplots(figsize=(5, 3), dpi=120)
    ...
    if 'Sales' in corr_matrix.columns:
        ...
        st.write("**Variables with highest correlation to Sales:**")
        for var, corr_val in sales_corr.head(5).items():
            st.write(f"- {var}: {corr_val:.3f}")
        st.write("**Insight:** Positive coefficients drive sales; negative coefficients inhibit sales. The larger the absolute value, the stronger the impact.")
        st.info("**Action:** Prioritize optimizing high-positive-correlation variables and controlling high-negative-correlation variables to boost sales.")
    else:
        # Fallback if 'Sales' not found (maybe original column name is '销售额' and mapping failed)
        # Try to find any column containing '销售额' or 'Sales'
        sales_col_candidates = [c for c in corr_matrix.columns if '销售额' in c or 'Sales' in c]
        if sales_col_candidates:
            sales_col = sales_col_candidates[0]
            sales_corr = corr_matrix[sales_col].drop(sales_col).sort_values(ascending=False)
            st.write(f"**Variables with highest correlation to {sales_col}:**")
            for var, corr_val in sales_corr.head(5).items():
                st.write(f"- {var}: {corr_val:.3f}")
            st.write("**Insight:** Positive coefficients drive sales; negative coefficients inhibit sales.")
            st.info("**Action:** Focus on optimizing high-positive-correlation variables.")
# 价格-销量散点
if "价格" in df_clean.columns and "销量" in df_clean.columns:
    ...
    if not df_price_sales.empty:
        ...
        ax.set_title("Price vs Quantity", fontsize=10)
        ax.set_xlabel("Price", fontsize=8)
...
if "折扣" in df_clean.columns:
    ...
    ax.set_title("Discount vs Sales", fontsize=10)
    ax.set_xlabel("Discount", fontsize=8)
    ax.set_ylabel("Sales", fontsize=8)
...
if age_col:
    ...
    if not df_age_valid.empty:
        ...
        ax.set_title("Age vs Avg Sales", fontsize=10)
        ax.set_xlabel("Age", fontsize=8)
        ax.set_ylabel("Avg Sales", fontsize=8)
...
''')
            ...
            axes[0].set_title("Actual vs Predicted", fontsize=9)
            axes[0].set_xlabel("Actual", fontsize=8)
            axes[0].set_ylabel("Predicted", fontsize=8)
            residuals = y_test - best_pred
            axes[1].scatter(best_pred, residuals, alpha=0.6, c='purple')
            axes[1].axhline(y=0, color='r', linestyle='--')
            axes[1].set_title("Residuals", fontsize=9)
...
if region_col and category_col:
    ...
    if not cross_data.empty and len(cross_data) <= 20:
        ...
        ax.set_title("Region × Category", fontsize=10)
        ax.set_xlabel("Region", fontsize=8)
        ax.set_ylabel("Sales", fontsize=8)
        plt.xticks(rotation=45, ha="right", fontsize=8)
        plt.tight_layout(pad=0.3)
...
if age_col and category_col:
    ...
    if not df_age_valid.empty:
        ...
        if not cross_age.empty:
            ...
            ax.set_title("Age × Category", fontsize=10)
            ax.set_xlabel("Age", fontsize=8)
            ax.set_ylabel("Sales", fontsize=8)
            plt.xticks(fontsize=8)
            plt.tight_layout(pad=0.3)
...
if channel_col and pay_col:
    ...
    if not cross_channel_pay.empty:
        ...
        ax.set_title("Channel × Payment", fontsize=10)
        ax.set_xlabel("Channel", fontsize=8)
...
if "价格" in df_clean.columns and "折扣" in df_clean.columns:
    ...
    if not df_price_valid.empty:
        ...
        if not cross_price.empty:
            ...
            ax.set_title("Price × Discount", fontsize=10)
            ax.set_xlabel("Price", fontsize=8)
            ax.set_ylabel("Sales", fontsize=8)
            plt.xticks(rotation=45, ha="right", fontsize=8)
            plt.tight_layout(pad=0.3)
